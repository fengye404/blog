...
最近我在学 CS336「Language Modeling from Scratch」，斯坦福的大语言模型构建课程。这门课在 X 上讨论度很高，中文社区里完整分享学习过程的却不多，所以想边学边写这个系列，把过程记录下来，也算自己的学习笔记。

课程名字里的 from Scratch 是从零手搓，课程本身不从零教学。讲义默认读者有神经网络和数学基础，对没有系统机器学习背景的同学来说，学起来要补不少课。这也是我写这个系列的原因：尽量把工程同学容易卡住的地方讲透。

先叠个甲：我之前的背景是纯工程开发，25 年初开始转向 Agent 开发，没有受过系统的机器学习训练。

这一篇是系列的第一篇，跟随 CS336 Assignment1 的思路，完整走一遍从零手搓 Transformer 的过程。为了先建立体感，开头会用我自己写的一组小 lab 热身，再进 A1 的实现。要手搓的不是原始论文版本，会引入 pre-norm、RMSNorm、RoPE、SwiGLU 这些变体，这也是 CS336 这门课的优秀之处。文中代码都在本机跑过，测试方法一并写出来。
...
这些问题的答案都在模型内部。我之前也是典型的黑盒派，把 LLM 当魔法 API 用，模型不听话的时候就靠试。理解了底层，至少知道该往哪个方向试。说白了，Agent 和 harness 工程的上限，取决于对模型行为的预判能力。
