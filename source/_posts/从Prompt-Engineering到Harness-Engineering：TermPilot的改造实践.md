...
`TermPilot` 是我业余时间做的一个项目。它最开始其实就是一次很典型的 `vibecoding` 实践：我一边想，一边让 `Codex` 往前写，很多功能、结构和验证脚本，都是这样一点点长出来的。

所以这篇文章并不是“我先理解了 harness engineering，再按这个理论去做一个新项目”。更接近真实情况的是：`TermPilot` 先在高强度 agent 协作里跑起来了，仓库里也慢慢长出了一些雏形，比如文档、协议边界、脚本化验证、架构说明。只是那时候这些东西还比较散，更多是为了把眼前的问题做完，还没有被我当成一套完整的方法来思考。

之前的开发方式，基本上还是 `prompt engineering` 那一套。也就是我脑子里知道项目怎么运转，需求来了以后，把上下文尽量讲清楚，让 agent 继续往下做；如果做偏了，就继续补背景；如果改动比较大，就靠自己记得住哪些地方要补文档、哪些脚本要重跑。它不是完全不能用，实际上 `TermPilot` 就是这么被做出来的，但这种方式很依赖“我本人一直在线、而且一直记得很清楚”。

这次真正引入 harness 的思路之后，我最大的感受不是“换了一套术语”，而是开发方式真的变了。以前是我带着 agent 往前走，现在更像是先把仓库整理成一个 agent 能自己进入、自己判断、自己验证的工作环境，再让它继续做事。很多原来靠临场解释、靠经验补洞的东西，慢慢都转成了仓库里的固定入口、计划、检查器和 CI。
...
所以这篇文章更像一次回头整理。我想讲的不是“如何从零设计一个 agent-first 项目”，而是一个已经靠 `Codex` 和 `vibecoding` 往前跑了很久的项目，怎么在后面这一步，慢慢长出更成体系的 harness；以及在这个过程中，我自己怎么从“把 prompt 写好”转向“把工程环境搭好”。
