...
我本职是 `Java` 服务端开发。`TermPilot` 不是日常工作的服务端项目，而是我业余时间写的一个端侧 `vibecoding` 项目。前期基本就是 `Codex + vibecoding` 的开发方式。功能先做出来，结构在开发过程中逐步成型，文档、脚本、协议边界也都有，但没有被系统化整理。
...
也正因为这个背景，我后来觉得这件事很有代表性。它虽然是一个个人端侧项目，但暴露出来的问题，并不只属于端侧开发。很多问题在已有的 `Java` 服务端仓库里同样存在，只是平时未必会被如此集中地放大出来。
...
如果把这个思路放回我更熟悉的 `Java` 服务端场景，其实也很容易落地。一个已有的服务端仓库，通常可以从下面几个地方开始：

1. 先把模块边界说清楚，例如 `controller`、`service`、`repository`、`client`、`domain` 分别负责什么，哪些依赖是允许的，哪些跨层调用是不允许的。
2. 把运行和调试入口固定下来，例如本地启动命令、集成测试命令、回归检查命令、常用环境变量和依赖服务准备方式。
3. 把关键业务约束写成 agent 能读懂的规则，例如事务边界、幂等要求、缓存一致性、消息投递语义、DTO 和领域对象的转换边界。
4. 把容易漂移的知识显式化，例如核心表结构约束、接口兼容性要求、外部依赖的调用约定、失败重试和降级策略。
5. 把最关键的回归路径沉淀成仓库内可直接运行的验证，例如单测、集成测试、契约检查、数据库迁移校验、接口 smoke test。

对服务端开发来说，harness engineering 带来的启发其实很直接：不要只想着“怎么让 agent 帮我写一个 controller 或 service”，而是先把这个服务端工程整理成 agent 能进入、能判断、能验证的系统。只有这样，后面的 vibecoding 才会越来越稳，而不是越写越乱。
