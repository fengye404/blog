...
现在回头看，我更愿意把 harness engineering 理解成一种工程层面的设计思路。重点不在某个单点工具，而在系统怎样组织入口、约束、验证和反馈。
...
这里的关键不是这些脚本名本身，而是把原来依赖个人环境的界面检查、端到端检查和兼容入口，全部收敛到仓库自己的验证路径里。这样 agent 能跑，CI 也能跑，验证能力不再附着在某一台开发机器上。
